...
class NangoService:
    ...
    async def patch_data(self, connection_id: str, endpoint: str, eventId: str, payload: Optional[Dict[str, Any]] = None, provider = 'salesforce') -> Dict[str, Any]:
        ...
        url = f"{self.base_url}/v1/{endpoint}"
        logger.info(f"Making Nango API PATCH request with connection_id: {connection_id}")
        ...
        try:
            ...
            response.raise_for_status()
            result = response.json()
            logger.info(f"Nango API PATCH request successful for endpoint: {endpoint}")
            return result
        except requests.exceptions.RequestException as e:
            error_msg = f"Error patching data to Nango: {str(e)}"
            if hasattr(e, 'response') and e.response is not None:
                error_msg += f" - Status code: {e.response.status_code}"
                try:
                    error_details = e.response.json()

...
from app.services.streaming_chunker import StreamingChunker
from app.adapters.openai_adapter import OpenAiAdapter
from app.helpers.utils import json_serial
...
class AIService:
    ...
    def add_message(self, conversation_id, role, content):
        ...
        self.conversations[conversation_id].append({"role": role, "content": content , "timestamp" : json_serial(datetime.now())})
        # Function to add messages to a conversation
    ...
    async def generate_response(self, conversation_id, message: str, synthesize_response, ai_client):
        ...
        if ai_client['name'] == 'OpenAI':
            assistant_reply = self.openai_adapter.generate_response(
        ...
        if (len(self.system_convo[conversation_id]) >= 6 + self.convo_index):
            ...
            if ai_client['name'] == 'OpenAI':
                summary = self.openai_adapter.generate_response(
                    model="gpt-4-turbo",
